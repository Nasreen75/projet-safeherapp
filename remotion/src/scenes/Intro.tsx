...
export const Intro = () => {
  ...
  const lineWidth = interpolate(spring({ frame: frame - 55, fps, config: { damping: 20 } }), [0, 1], [0, 200]);
  const glowScale = 1 + Math.sin(frame / 15) * 0.1;

  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", fontFamily }}>
      <div style={{ position: "absolute", width: 280, height: 280, borderRadius: "50%", background: "radial-gradient(circle, hsla(346, 77%, 55%, 0.4), transparent 70%)", transform: `scale(${glowScale * shieldScale})` }} />
      <div style={{ transform: `scale(${shieldScale})`, marginBottom: 40 }}>
      ...
      <div style={{ transform: `translateY(${titleY}px)`, opacity: titleOpacity, textAlign: "center" }}>
        <span style={{ fontSize: 88, fontWeight: 800, color: "white", letterSpacing: -2 }}>Safe</span>
        <span style={{ fontSize: 88, fontWeight: 800, color: "hsl(346, 77%, 55%)", letterSpacing: -2 }}>Her</span>
      </div>
      <div style={{ width: lineWidth, height: 3, background: "linear-gradient(90deg, transparent, hsl(346, 77%, 55%), transparent)", borderRadius: 2, marginTop: 20, marginBottom: 20 }} />
      <div style={{ opacity: subtitleOpacity, transform: `translateY(${subtitleY}px)`, fontSize: 30, color: "hsla(0, 0%, 100%, 0.7)", fontWeight: 500, letterSpacing: 4, textTransform: "uppercase" }}>
